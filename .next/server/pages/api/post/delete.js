...
async function handler(request, response) {
    if (request.method == "POST") {
        let session = await (0,next_auth__WEBPACK_IMPORTED_MODULE_2__.getServerSession)(request, response, _auth_nextauth___WEBPACK_IMPORTED_MODULE_3__.authOptions);
        // console.log(session, "세션");
        /// auth 부분
        ////////////////////////////////
        /// db 부분
        const db = (await _util_database__WEBPACK_IMPORTED_MODULE_0__/* .connectDB */ .u).db("subi");
        let preResult = await db.collection("post").findOne({
...
var __webpack_exports__ = __webpack_require__.X(0, [270], () => (__webpack_exec__(46505)));
